...
## Phase 1: The Go Backend Migration (Complete)
...
* **PTY**: Implemented in Go (`creack/pty`). Wails bridge handles spawn, resize, write, and async data streams. ✅
* **Serial**: Implemented in Go (`go.bug.st/serial`). Wails bridge handles port listing, connect, write, and async data streams. ✅
* **SSH**: Full SSH client with auth, host key verification, keyboard-interactive, jump hosts. ✅
* **Telnet**: Implemented with go standard library. ✅
* **Session Persistence**: Save/restore tabs with working directories and connection details. ✅
* **Audit Logging**: Connection events logged to file. ✅
* **Communication**: Wails binding bridge. ✅

**Note:** The legacy Electron/Angular codebase remains as a reference but is NOT the active development target.

## Phase 1.5: Wails Go UI (Active)

**Priority: HIGH** | **Status: ACTIVE**

### Goal

Build a complete, self-contained Wails Go application with feature parity to the original Tabby terminal, using Go backend + HTML/JS frontend (no Angular, no Electron).

### Current Status

* **Custom Titlebar**: Frameless window with minimize/maximize/close. ✅
* **Session Persistence**: Save/restore tab states. ✅
* **Terminal Broadcast**: Send input to all tabs (Ctrl+Shift+B). ✅
* **Session Logging**: Capture and view/download terminal output. ✅
* **Profile Group Dropdown**: Datalist-based group suggestions in profile editor. ✅
* **Settings Descriptions/Tooltips**: Explanatory text for all config options. ✅

### Remaining

* **Serial Terminal enhancements**: Hex view mode, advanced flow control, connection logging
* **Profile Group Management**: Full group editor UI (rename, delete groups)
* **SSH Config Import polish**: Better error handling, merge strategy
